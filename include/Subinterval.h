...
// subinterval_state::ready_to_run means ivydriver has finished preparing the next subinterval by setting
...
// and that the WorkloadThread has declared the Subinterval ready for ivydriver to pick up and send to ivymaster.

// The ivydriver cycle starts with ivydriver waiting to the scheduled subinterval end time.
...
// When ivydriver harvests a subinterval, it gets the lock and checks the subinterval's status.
...
// In this case, ivydriver will wait for a quarter-catnap and try to get the lock again to see if in the mean time
...
// And when either ivydriver or the WorkloadThread has dropped the lock, it does a notify_all()
...
{
    undefined,
    ready_to_run,  // ivydriver finished setting up
